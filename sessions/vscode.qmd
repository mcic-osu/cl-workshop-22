...
should be done not on login nodes but on compute nodes.
...
(the session will actually stop working after the allotted time has
passed),
...
- A bunch of icons in the middle that serve to switch between
  different options for the **wide side bar** (to the right of the narrow side
  bar), which can show:
...
click &nbsp; <i class="fa fa-cog"></i> &nbsp; and then `Color Theme`.
(I like "*Quiet Light*".)
...
as the starting point for your operations.

:::{.callout-tip}
## Folder vs. directory

"Folder" and "directory" mean the same thing --
the latter is most commonly used in the context of the Unix Shell.

:::

`VS Code` has a concept of a working directory that is
**effective in all parts of the program**:
...
`/fs/ess/scratch/PAS2250` (you'll make personal folders within there shortly).
...
It will reopen the text files that you had open and if you had an active terminal,
it will also open a terminal.
This is very convenient, especially when you start working on multiple projects
(different folders) in `VS Code` and switch between those.
...
(for Mac, replace <kbd>Ctrl</kbd> by <kbd>⌘</kbd>).
