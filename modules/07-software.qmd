...
which is `conda install -c bioconda cutadapt` here.
...
Since our strategy here (and my general strategy) to create a new environment
for each program,
we would therefore replace `install` by `create -y -n <env-name>`,
such that our full command (without version specification) again becomes:
